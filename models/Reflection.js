const pool = require("../config/connection");

class Reflection {
	constructor(
		id,
		success,
		low_point,
		take_away,
		owner_id,
		created_date,
		modified_date
	) {
		this.id = id;
		this.success = success;
		this.low_point = low_point;
		this.take_away = take_away;
		this.owner_id = owner_id;
		this.created_date = created_date;
		this.modified_date = modified_date;
	}

	static getAll(owner_id) {
		return new Promise((resolve, reject) => {
			pool
				.query(
					`SELECT id, success, low_point, take_away, owner_id, created_date, modified_date
                    FROM reflections_tab WHERE owner_id = $1;`,
					[owner_id]
				)
				.then(({ rows }) => {
					// fetching data seperti @foreach di php
					const reflection = rows.map((row) => {
						return new Reflection(
							row.id,
							row.success,
							row.low_point,
							row.take_away,
							row.owner_id,
							row.created_date,
							row.modified_date
						);
					});
					resolve(reflection);
				})
				.catch((err) => {
					reject(err);
				});
		});
	}

	static create(success, low_point, take_away, owner_id) {
		return new Promise((resolve, reject) => {
			const now = new Date();
			pool
				.query(
					`INSERT INTO reflections_tab 
                        (success, low_point, take_away, owner_id, created_date, modified_date) 
                        VALUES ($1, $2, $3, $4, $5, $6) 
                        RETURNING id, success, low_point, take_away, owner_id, created_date, modified_date;`,
					[success, low_point, take_away, owner_id, now, now]
				)
				.then(({ rows }) => {
					const reflection = new Reflection(
						rows[0].id,
						rows[0].success,
						rows[0].low_point,
						rows[0].take_away,
						rows[0].owner_id,
						rows[0].created_date,
						rows[0].modified_date
					);
					resolve(reflection);
				})
				.catch((err) => {
					reject(err);
				});
		});
	}

	static update(success, low_point, take_away, reflection_id) {
		return new Promise((resolve, reject) => {
			const now = new Date();
			pool
				.query(
					`UPDATE reflections_tab 
                        SET success=$1, low_point=$2, take_away=$3, modified_date=$4 WHERE id=$5
                        RETURNING id, success, low_point, take_away, owner_id, created_date, modified_date;`,
					[success, low_point, take_away, now, reflection_id]
				)
				.then(({ rows }) => {
					const reflection = new Reflection(
						rows[0].id,
						rows[0].success,
						rows[0].low_point,
						rows[0].take_away,
						rows[0].owner_id,
						rows[0].created_date,
						rows[0].modified_date
					);
					resolve(reflection);
				})
				.catch((err) => {
					reject(err);
				});
		});
	}

	static delete(reflection_id) {
		return new Promise((resolve, reject) => {
			const now = new Date();
			pool
				.query(
					`DELETE FROM reflections_tab 
                        WHERE id = $1
                        RETURNING id, success, low_point, take_away, owner_id, created_date, modified_date;`,
					[reflection_id]
				)
				.then(({ rows }) => {
					const reflection = new Reflection(
						rows[0].id,
						rows[0].success,
						rows[0].low_point,
						rows[0].take_away,
						rows[0].owner_id,
						rows[0].created_date,
						rows[0].modified_date
					);
					resolve(reflection);
				})
				.catch((err) => {
					reject(err);
				});
		});
	}
}

module.exports = Reflection;
